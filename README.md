...
配合 GrokIQ 回调实现"注册 → 编排内号池 → 降智检测 → 推送生产号池"隔离闭环：

- **判定（严格口径）**：回调 `degraded=false` 且 `isolated=false` 且 `probe_outcome=passed` 才推送；其余情况仅记录原因
- **代理出口保护**：推送前取该账号在编排内号池绑定的出口节点名末段 SID（`...-<sid>`），在生产号池同 scope 出口节点中匹配同 SID 节点绑定（manual）；未命中则随机绑定可用节点；每域（Build/Web/Console）独立执行
- **staging 处置**：全部选定域推送成功后自动禁用编排内同账号，避免同一 SSO 双池活跃
- **可靠性**：持久队列 + 后台 worker + 指数退避重试（≤5 次），重启自动恢复
- **配置**：「注册设置 → 授权转换 → Grok2API → 正式号池推送」卡片（总开关、生产 API 地址/账密、三域开关联动、出口保护开关）
- **展示**：账号管理详情新增「正式Grok2API」「代理出口守护情况」（域徽章：绿=SID 命中、红=未命中/失败、黄=未选定），列表行含"正式已推正式"徽章

生产号池的出口节点需按相同命名约定导入（名称末段为 SID）。
